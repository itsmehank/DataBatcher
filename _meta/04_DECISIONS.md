...
> **참고 (2026-05-11 추가)**: 본 역할 정의(특히 Auditor 필수 가동)는 ADR-016에 의해 부분 supersede됨. ADR-016 §3 (조건부 Auditor) 및 §4 (등급 판정 권한) 적용 시 ADR-016 우선.
...
## ADR-016 — 위험 3등급제 + 조건부 Auditor + 등급 판정 권한

**Status**: Accepted (2026-05-11)

### §1. 배경

Phase 1 종료 후 Phase 2A (메일+엑셀) 진행에서 거버넌스 오버헤드가 본 작업보다 무거운 패턴 관찰:
- Sprint 1 (엑셀) 본 작업 1~2일 + 거버넌스 작업 2~3일
- Sprint F #7 (거버넌스 사실 정정) 별도 sprint 발생
- 4 세션 동안 실제 기능 1건 (엑셀) 추가

원인: 모든 작업에 동일 절차 적용 (Architect brief + Auditor 헌법 5종 + ADR 검토 + 거버넌스 갱신). 헌법 위험 영역과 잡일이 같은 무게의 절차로 묶임.

### §2. 결정 — 위험 3등급제

모든 작업을 다음 3등급으로 분류, 등급별 절차 적용.

| 등급 | 정의 | 절차 |
|---|---|---|
| **A급** | 헌법 §2.1·§2.2·§2.5·§3.1·§4 위험 영역. 예: LLM→주문 경로, 4계층 단방향 변경, 결정론 코어와 LLM 분리, LLM 출력 영구 보존 매체 변경, 사용자 통제권 변경 | 현 절차 유지. Architect brief + Auditor 헌법 5종 + ADR 채택 + 거버넌스 일괄 갱신 |
| **B급** | 기능 변경. 예: prompt 변경, 분류 로직, DB 스키마, 새 스크리너, 새 분석 모듈, 새 외부 데이터 소스 | Builder 단독 진행 + PR 본문에 사유 1단락 + 사용자 승인. ADR 채택만 (별도 brief sprint 단위 분할 없음) |
| **C급** | 잡일·후처리. 예: 메일·엑셀 템플릿, 문서 오타, 운영 큐 처리, 의존성 버전 갱신, 코드 리팩터 | Builder 직접 처리. 절차 없음. PR 또는 직접 commit |

### §3. 결정 — 조건부 Auditor

Auditor 세션은 본 Phase가 헌법 §2.1·§2.2·§2.5 중 어느 항목이라도 건드린 경우만 호출.

대안 절차 (헌법 영역 무관 Phase):
- Phase 종료 시 Builder가 "본 Phase는 §2.1·§2.2·§2.5 어느 항목도 건드리지 않음" 선언
- Architect가 1단락 확인
- 정식 Auditor 세션 생략

Phase 1 Auditor 결과(PASS 5/5, FAIL 0, FINDING 2건 모두 명문화 누락)로 비용 대비 효익 검증.

### §4. 결정 — 등급 판정 권한 + 안전망

등급 자가 분류 회피 방지를 위한 권한 매트릭스:

- **C급 자가 분류 허용** — Builder가 본인 작업을 C급으로 자가 분류 가능
- **B급 분류는 Architect 또는 사용자가 1회 확인**
- **A급은 자동 분류** — 헌법 본문에 정의된 영역(§2.1·2.2·2.5·§3.1·§4) 침범 시 회피 불가. 발견 시 즉시 A급 절차로 격상.

판정 우선순위: **자동 A급 > Architect/사용자 B급 확인 > Builder 자가 C급**

#### §4.1 안전망 — C급 자가 분류 발견 경로

C급 자가 분류 + 조건부 Auditor 미가동 결합 시 A급 영역 침범 발견 경로 부재 위험. 다음 의무로 보완:

1. **C급 작업이 다음 코드 영역을 건드릴 때 PR 사유 1단락에 헌법 영역 비건드림 명시**:
   - `apps/llm-analysis/` 디렉터리 내 LLM 호출 경로 (계층 2)
   - `apps/llm-analysis/core/result_parser.py` (LLM 출력 → DB 영구 보존 경로)
   - `apps/ingest-databatcher/` 디렉터리 (계층 1 결정론 코어)
   - `daily_analysis_kr/us`, `llm_calls` 테이블 스키마
   - 외부 발송·실행 경로 (현 시점은 SMTP, Phase 3 이후 추가 가능)

2. PR 사유 1단락 양식:
   ```
   본 작업의 헌법 영역 점검:
   - §2.1 (LLM 직접 주문 실행): 비건드림 / 건드림 + 사유
   - §2.2 (결정론 코어·LLM 분리): 비건드림 / 건드림 + 사유
   - §2.5 (LLM 출력 영구 보존): 비건드림 / 건드림 + 사유
   ```

3. PR 사유 1단락은 사용자 또는 Architect 사후 review에서 검토. 건드림 케이스 발견 시 B급 또는 A급 절차로 격상.

4. 본 의무를 누락한 C급 PR은 사후 review 대상으로 표시 — 머지 전 사용자가 §4.1 1단락 보충 요구.

### §5. ADR-005와의 관계

ADR-016은 **ADR-005 §3 본문의 부분 supersede**다 (단순 부수 항목 추가 아님 — ADR-014 §1(a) 결정 본문 변경 카테고리).

- ADR-005 §3 (a) Architect 권한 — A급·B급에서 유지. C급에서는 Builder 자율
- ADR-005 §3 (b) Builder 권한 — 변경 없음. 등급별 작업 진행 권한 추가
- ADR-005 §3 (c) Auditor 권한 — 필수 가동 → **조건부 가동으로 본문 supersede** (본 ADR §3)

ADR-005 자체는 폐지하지 않고, ADR-005 §3 본문 옆에 "ADR-016에 의해 부분 supersede됨, 본 ADR 적용 시 ADR-016 우선" 메타 주석 1줄 추가 (Builder가 본 ADR 채택 시 동시 적용 — §2 작업 2).

### §6. 영향 — Phase 2 진행 중 sprint 등급 분류

| Sprint | 등급 | 사유 |
|---|---|---|
| Sprint 1 (엑셀) — 완료 | C급 (소급) | 잡일·후처리, 헌법 영역 무관 |
| Sprint 2 (SMTP 메일) — 진입 | C급 | 잡일·후처리. SMTP는 LLM 호출·LLM 출력 보존 경로와 무관. 발송 기록 JSONL은 운영 추적용 (헌법 §4 사용자 통제권 부수) |
| Sprint 3 (자동 발송 스케줄) | C급 | Task Scheduler 등록, 헌법 영역 무관 |
| Sprint C (ADR-015 fund vehicle) | B급 | 분류 로직 변경 |
| Sprint A (entry 자연 누적) | B급 | 평가 자동화 |
| Sprint B (Evaluator 6종) | B급 | prompt·분류 변경 |
| Sprint D (분류 안정성) | B급 | 분석 스크립트 |
| Sprint E (prompt v3) | B급 | prompt 변경 |
| Sprint F (운영 부수) | C급 (대부분) | 잡일 |

### §7. 영향 — Phase 2 종료 게이트 변경

본 ADR 채택과 동시에 `phase2_brief.md` §9.1 본문 수정. Phase 2 종료 차단 조건은 Sprint 1·2·3 (Phase 2A 본질)과 헌법 §2.5 준수만. Sprint A·B·C·D·E·F는 maintenance backlog로 분리, Phase 3 진입 차단 조건 아님 (별도 만료일·후행 처리 부여 — `phase2_brief.md` §9.1-bis 표 참조).

### §8. 영향 받지 않는 항목

- 헌법 본문 (§2.1·§2.2·§2.5·§3.1·§4) — 변경 없음
- ADR-010 (DB 스키마 변경 3종 산출물) — A·B급에 한정 적용 유지
- ADR-013·015 (fund vehicle 분류) — B급 분류 절차 적용
- 단위 테스트 회귀 0건 원칙 — 모든 등급에 적용

### §9. 채택일·검토일

- 채택일: 2026-05-11 (사용자 승인 + 본 commit)
- 첫 검토일: Phase 2A 종료 시점 — 등급제 운영 정착 여부 점검
- 재검토 트리거: Builder가 C급 자가 분류한 작업에서 헌법 영역 위험 사후 발견 시, 또는 §4.1 1단락 누락 PR 3건 이상 누적 시

---

*새로운 결정이 있을 때마다 ADR-017, ADR-018... 형태로 추가한다.*
