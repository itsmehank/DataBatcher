...
### Q-004: us_symbol_master ETF 정정 12건 (등록: 2026-05-08, 완료: 2026-05-10)

**상태**: 자연 완료 (UPDATE 실행 불필요)

**완료 사유**:
2026-05-10 PROD 사전 확인 결과 12건(VRTL, SOXL, MVLL, MUU, MULL, AMDG, AMDL, AMUU, KORU, INTW, DLLL, BWET) 모두 이미 `symbol_type = 'ETF'` 자연 정정 확인. UPDATE 실행 불필요.

**자연 정정 원인 추정**:
- us_sync_symbol_master.py 정기 실행 시 외부 소스(FDR) 분류 갱신
- 또는 외부 소스 자체의 분류 정보 정정
- 정확한 원인 추적은 ADR-015 Sprint C (Phase 2B)에서 us_sync_symbol_master.py 보강 시 분석

**검증**:
- PROD `SELECT symbol, symbol_type FROM us_symbol_master WHERE symbol IN (...12종...)` 결과 12건 모두 'ETF' 확인
- 다음 Minervini 스크리너 실행 시 12건 모두 제외됨 (ADR-013 안전망과 별도로 us_symbol_master 단계에서 사전 제외)

**§10.4 1번 예외 적용**:
본 큐 항목은 phase1_brief §10.4-bis 1번 예외 적용으로 1.3.10 단계 한정 Builder가 직접 등록(2026-05-08). 본 완료 처리는 Architect 세션이 정식 절차로 처리.

**ADR-015 Sprint C 관계**:
Q-004 자연 완료가 ADR-015 Sprint C를 폐지하지 않음. ADR-015 §4 (Fund Vehicle 4 카테고리 명확화 + override 테이블 + 안정성 보강)는 Phase 2B에서 그대로 진행.
