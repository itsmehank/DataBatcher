...
def main(argv=None) -> int:
    ...
    if etf_excluded:
        print(f"[kr_minervini_update] ADR-013: ETF {etf_excluded}종목 제외 (symbol_type='ETF')")
    ...
    print(f"[kr_minervini_update] 종목 수(ETF 제외): {len(symbol_details)}")
    ...
    if etf_price_cols:
        ...
        print(f"[kr_minervini_update] ADR-013: 가격 데이터 ETF 컬럼 {len(etf_price_cols)}개 제거")
