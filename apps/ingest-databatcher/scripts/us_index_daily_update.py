...
실행 예시:
    python scripts/us_index_daily_update.py --all
    python scripts/us_index_daily_update.py --all --market SP500
    python scripts/us_index_daily_update.py --symbols US500 DJI
"""
...
# us_index_prices 테이블에서 조회할 컬럼 (adj_close 없음)
INDEX_PRICE_COLUMNS = ["date", "open", "high", "low", "close", "volume"]


def build_pipeline(cfg) -> IndicatorPipeline:
    specs_cfg = cfg.get("indicators_us_index", {}).get("pipeline", [])
...
def extract_indicators_for_date(
    ...
    if not df_long.empty and 'value' in df_long.columns:
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="US Index 일일 업데이트")
    p.add_argument("--symbols", nargs="*", default=None,
                   help="수집할 지수코드 리스트 (미지정 시 --all 필요)")
    p.add_argument("--end", default=None,
                   help="수집 종료일(YYYY-MM-DD). 미지정 시 ET cutoff 기준")
...
def main(argv=None):
    ...
    if not args.symbols and not args.all:
        ...
        print("  python scripts/us_index_daily_update.py --all", file=sys.stderr)
        print("  python scripts/us_index_daily_update.py --symbols US500 DJI", file=sys.stderr)
        sys.exit(2)
    ...
    end_date = pd.to_datetime(args.end).date() if args.end else default_end_date_us_eastern_cutoff(cutoff_hhmm)

    # 지수 목록 결정
    if args.all:
        indices = load_us_index_symbols_with_details(engine, market=args.market)
        if not indices:
            print("Error: us_index_master에서 ACTIVE 지수를 찾을 수 없습니다.", file=sys.stderr)
            print("먼저 'python scripts/us_index_sync_master.py'를 실행하세요.", file=sys.stderr)
            sys.exit(2)
        print(f"Loaded {len(indices)} indices from us_index_master (market={args.market})")
    else:
        ...
        for sym in args.symbols:
            ...
            with engine.connect() as conn:
                ...
                if row:
                    ...
                    print(f"Warning: {sym} not found in us_index_master, skipping", file=sys.stderr)
    ...
    checker = IndicatorChecker(engine, cfg, table="us_index_indicators", config_key="indicators_us_index")
