#!/usr/bin/env python3
"""
US Index 주봉 증분 업데이트 스크립트

처리 흐름:
1. DB에서 최근 250주(~1750일) 일봉 데이터 조회 (us_index_prices)
2. 주봉으로 집계
3. 주봉 가격 데이터 INSERT ONLY (us_index_prices_weekly)
4. warmup 포함 확장 기간 일봉 재조회 → 주봉 집계 → 지표 계산
...
주의:
    ...
    - 매주 토요일 실행 권장 (금요일 US 장마감 데이터 확정 후)
    - US 시장 기준이므로 ET (Eastern Time) 기준으로 주말 판단
"""
...
def build_pipeline(cfg) -> IndicatorPipeline:
    """US Index 주봉 지표 파이프라인 구성"""
    weekly_cfg = cfg.get("indicators_us_index_weekly", {}) or {}
    if not isinstance(weekly_cfg, dict):
        raise ValueError("[us_index_weekly_update] indicators_us_index_weekly 설정은 dict 형태여야 합니다.")

    specs_cfg = weekly_cfg.get("pipeline") or []
    if not isinstance(specs_cfg, list):
        raise ValueError("[us_index_weekly_update] indicators_us_index_weekly.pipeline 설정은 list 형태여야 합니다.")
...
def load_daily_data(engine: Engine, symbol: str, start_date: date, end_date: date) -> pd.DataFrame:
    """
    DB에서 US Index 일봉 데이터 조회 (adj_close 없음)
...
def check_weekly_indicators_complete(
    engine: Engine, symbol: str, week_start: str, market: str, expected_count: int
) -> bool:
    """특정 (symbol, week_start, market)에 대해 'row 수 >= expected spec 수'면 완료로 간주."""
    sql = text(
        """
        SELECT COUNT(*) as cnt
        FROM us_index_indicators_weekly
...
def process_index(
    ...
    """
    단일 US 지수의 주봉 증분 업데이트

    Contract:
    - 입력 범위: end_date 기준 최근 250주 일봉 + warmup 확장 구간
    - 저장 정책: 가격/지표 모두 insert_only (기존 데이터 덮어쓰기 없음)
    - 중단 조건: 최신 주부터 역순 처리 중 해당 주 지표가 이미 완료면 즉시 중단
...
def should_skip_latest_week_et() -> bool:
    """US Eastern Time 기준으로 평일에는 최신 주차를 스킵, 주말(토/일)에는 포함.
    ...
    dow = now_et.weekday()  # Monday=0 ... Sunday=6
    return dow < 5


def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="US Index 주봉 증분 업데이트")

    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--all", action="store_true", help="전체 지수 업데이트")
    group.add_argument("--symbols", nargs="+", help="특정 지수만 (예: US500 DJI)")

    parser.add_argument(
        "--market",
        choices=["SP500", "DJI", "IXIC", "ALL"],
        default="ALL",
        help="시장 필터 (기본값: ALL)"
    )
...
def main(argv=None) -> int:
    ...
    # Get end_date (US 시장 기준)
    date_utils = DateUtils('America/New_York')
    end_date = date_utils.get_last_business_day('NYSE')

    skip_latest_week = should_skip_latest_week_et()
    print(f"[us_index_weekly_update] skip_latest_week={skip_latest_week} (ET weekend -> False)")
    print(f"[us_index_weekly_update] 기준일: {end_date}")

    # Load indices
    if args.all:
        indices = load_us_index_symbols_with_details(engine, market=args.market)
        print(f"[us_index_weekly_update] 처리 대상: {len(indices)}개 지수")
    else:
        indices = []
        for sym in args.symbols:
            sql = "SELECT symbol, name, market FROM us_index_master WHERE symbol = :symbol AND status = 'ACTIVE'"
            with engine.connect() as conn:
                ...
                if row:
                    ...
                    print(f"Warning: {sym} not found in us_index_master (or not ACTIVE), skipping", file=sys.stderr)

        print(f"[us_index_weekly_update] 처리 대상: {len(indices)}개 지수")

    if not indices:
        print("[us_index_weekly_update] 처리할 지수가 없습니다")
        return 0

    # Build US Index weekly pipeline
    ...
    print("[us_index_weekly_update] 완료")
    return 0
