...
# -*- coding: utf-8 -*-
"""
미국 주식 미너비니 트렌드 템플릿 스크리닝 배치

처리 흐름:
  1. us_symbol_master에서 ACTIVE 종목 목록 로드
  2. us_stock_prices에서 전 종목 가격 로드 (wide format, ~252+days+60)
  3. us_stock_indicators에서 RS Rating, Blue Dot 로드 (이미 계산된 것 재사용)
  4. 미너비니 트렌드 템플릿 스크리닝 수행
  5. 통과 종목만 minervini_screen_results_us에 INSERT ONLY 저장

실행 예:
    python scripts/us_minervini_update.py --days 9999  # 전체 백필
    python scripts/us_minervini_update.py --days 7     # 최근 7일
    python scripts/us_minervini_update.py --market NASDAQ --days 30
"""
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="US 주식 미너비니 트렌드 템플릿 스크리닝 배치")
...
def main(argv=None) -> int:
    ...
    # 미너비니 설정 로드
    minervini_cfg = cfg.get("minervini_us", {})
    minervini_price_column = minervini_cfg.get("price_column", "adj_close")
    filters_cfg = minervini_cfg.get("filters", {})
    screen_hash = params_hash("minervini_screen", minervini_cfg)

    # 종료일 (US Eastern cutoff 기준)
    ...
    print(
        f"[us_minervini_update] market={args.market} end={end_d} days={args.days} "
        f"price_column={minervini_price_column}"
    )
    print(f"[us_minervini_update] screen_config_hash={screen_hash}")
    print(f"[us_minervini_update] load_start={load_start}")
    ...
    prices_wide = load_all_prices_wide(
        ...
        price_column=minervini_price_column,
    )
    ...
    if filters_cfg.get("rs_rating", {}).get("enabled", True):
        ...
        if rs_rating_wide.empty:
            print(
                ...
                "us_rs_update.py를 먼저 실행하세요.",
                file=sys.stderr,
            )
            return 5
        else:
            print(f"[us_minervini_update] RS Rating: {rs_rating_wide.shape[0]} 거래일 x {rs_rating_wide.shape[1]} 종목")
    ...
    if filters_cfg.get("blue_dot", {}).get("enabled", False):
        ...
        if blue_dot_wide.empty:
            ...
            print(f"[us_minervini_update] Blue Dot: {blue_dot_wide.shape[0]} 거래일 x {blue_dot_wide.shape[1]} 종목")
    ...
    print(f"[us_minervini_update] 스크리닝 완료: {pass_mask.shape[0]} 거래일 x {pass_mask.shape[1]} 종목")
    ...
    print(f"[us_minervini_update] 저장 대상: 최근 {len(save_dates)} 거래일")
    ...
    if rs_rating_wide is not None and not rs_rating_wide.empty:
        ...
        if missing:
            print(f"WARNING: RS Rating 누락 날짜 {len(missing)}일 발견. us_rs_update.py를 먼저 실행하세요.")
    ...
    DBManager.upsert_dataframe(
        engine=engine,
        df=result_df,
        table="minervini_screen_results_us",
        mode="insert_only",
        unique_keys=("symbol", "date", "screen_config_hash"),
    )

    print("[us_minervini_update] 완료")
    return 0
