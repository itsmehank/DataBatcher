...
#
# 용도: 서비스 최초 구축 시 DB 초기화 + 2020.01부터 현재까지 전체 수집
...
# 2단계: DB 스키마 초기화
log "[2/6] DB 스키마 초기화 중..."
python -m src.real_estate.cli init-db 2>&1 | tee -a "$LOG_FILE"
log "[2/6] DB 스키마 초기화 완료"

# 3단계: 데이터 수집 (2020.01 ~ 현재)
CURRENT_YM=$(date +%Y%m)
START_YM="202001"
log "[3/6] 데이터 수집 시작 (${START_YM} ~ ${CURRENT_YM}, 서울 10개구)"
