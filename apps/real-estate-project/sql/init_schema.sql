-- =============================================================================
-- init_schema.sql — 전체 테이블 스키마 정의
--
-- 용도: 프로젝트에서 사용하는 모든 테이블을 한 파일에서 관리합니다.
--       `python -m src.real_estate.cli init-db` 실행 시 동일한 테이블이 생성되지만,
--       이 파일은 스키마의 단일 참조 문서(SSOT) 역할을 합니다.
--
-- 실행: mysql -u <user> -p <database> < sql/init_schema.sql
-- 주의: 모든 DDL은 IF NOT EXISTS이므로 기존 테이블에 영향 없이 안전하게 재실행 가능합니다.
-- =============================================================================
