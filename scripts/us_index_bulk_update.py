#!/usr/bin/env python3
"""
US Index 과거 데이터 일괄 수집 스크립트

us_index_master의 ACTIVE 지수를 읽어 과거 OHLCV 데이터를 일괄 수집합니다.
지수는 3개뿐이므로 병렬 처리 없이 순차 처리합니다.
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        description="US Index 과거 데이터 일괄 수집 (Bulk Update)",
...
def main(argv=None):
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("DataBatcher US Index Bulk Update")
        logger.info("=" * 60)
        ...
        # Collect each index (sequential, only 3 indices)
        results = []
