...
# -*- coding: utf-8 -*-
"""
미국 주식(ETF 포함) IBD RS Rating + RS Line + Blue Dot 일괄 계산/저장

전 종목 횡단면(cross-sectional) 기반 지표이므로 기존 per-symbol 파이프라인이 아닌
별도 배치 스크립트로 구현. 저장은 기존 us_stock_indicators 테이블(long-form)을 재사용한다.
...
처리 흐름:
  ...
  5. RS Line 계산 (각 종목 vs S&P 500)
  6. Blue Dot 계산 (RS Line + 주가 기반)
  7. 최근 N일에 대해 long-form 변환 → us_stock_indicators에 INSERT ONLY 저장

실행 예:
    python scripts/us_rs_update.py
    python scripts/us_rs_update.py --days 5
    python scripts/us_rs_update.py --market NASDAQ --days 30
"""
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="US 주식 IBD RS Rating / RS Line / Blue Dot 배치")
    p.add_argument("--end", default=None,
                   help="기준 종료일 (YYYY-MM-DD). 미지정 시 ET cutoff 기준")
    p.add_argument("--days", type=int, default=30,
                   help="저장 대상 일수 (기본: 30, 최근 N 거래일)")
    p.add_argument("--market", choices=["NYSE", "NASDAQ", "ETF", "ALL"],
                   default="ALL", help="대상 마켓 (기본: ALL)")
    p.add_argument("--with-indicators", action="store_true", default=True,
                   help="호환용 (기본 True)")
...
def to_long_form(
    ...
    """wide-format DataFrame을 long-form(us_stock_indicators 형식)으로 변환."""
    ...
    melted["market"] = melted["symbol"].map(symbol_market_map).fillna("NYSE")
...
def main(argv=None) -> int:
    ...
    benchmark_symbol = ibd_cfg.get("benchmarks", {}).get("us", "US500")
    us_price_column = ibd_cfg.get("us_price_column", "adj_close")

    # 종료일
    cutoff_hhmm = cfg.get("markets", {}).get("us_daily_cutoff", "18:00")
    end_d: date = pd.to_datetime(args.end).date() if args.end else default_end_date_us_eastern_cutoff(cutoff_hhmm)

    # DB 로드 범위
    load_start = end_d - timedelta(days=WARMUP_DAYS + args.days + 60)

    print(
        f"[us_rs_update] market={args.market} end={end_d} days={args.days} "
        f"strict_12m={strict_12m} price_column={us_price_column}"
    )
    print(f"[us_rs_update] benchmark={benchmark_symbol} load_start={load_start}")

    # 1. 종목 목록 로드
    symbol_details = load_us_symbols_with_details(engine, market=args.market)
    ...
    print(f"[us_rs_update] 종목 수: {len(symbol_details)}")

    # 2. 전 종목 가격 로드 (wide format)
    ...
    print(f"[us_rs_update] 저장 대상: {len(save_dates)}일 ({save_dates[0].date()} ~ {save_dates[-1].date()})")
    ...
    for label, df_long in [("ibd_rs_rating", df_rs_rating), ("rs_line", df_rs_line), ("blue_dot", df_blue_dot)]:
        if df_long is not None and not df_long.empty:
            ...
            print(f"[us_rs_update] {label}: {rows}건 저장")
            total += rows
        else:
            print(f"[us_rs_update] {label}: 저장할 데이터 없음")

    print(f"[us_rs_update] 완료. 총 {total}건 저장")
    return 0
