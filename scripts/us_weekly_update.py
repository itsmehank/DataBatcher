#!/usr/bin/env python3
"""
US 주식 주봉 증분 업데이트 스크립트

처리 흐름:
1. DB에서 최근 250주(~1750일) 일봉 데이터 조회
2. 주봉으로 집계
...
실행 예시:
    ...
    python scripts/us_weekly_update.py --all --market NASDAQ --top 100
...
def build_us_weekly_pipeline(cfg) -> IndicatorPipeline:
    """US 주봉 지표 파이프라인 구성"""
    weekly_cfg = cfg.get("indicators_us_weekly", cfg.get("indicators_weekly", cfg.get("indicators", {}))) or {}
    if not isinstance(weekly_cfg, dict):
        raise ValueError("[us_weekly_update] indicators_us_weekly 설정은 dict 형태여야 합니다.")

    specs_cfg = weekly_cfg.get("pipeline") or []
    if not isinstance(specs_cfg, list):
        raise ValueError("[us_weekly_update] indicators_us_weekly.pipeline 설정은 list 형태여야 합니다.")
...
def save_weekly_prices_insert_only(engine: Engine, symbol: str, weekly_df: pd.DataFrame, market: str) -> int:
    """US 주봉 가격 데이터를 DB에 INSERT ONLY로 저장.
...
def parse_args(argv=None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="US 주봉 증분 업데이트")

    # 종목 선택 옵션
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--all", action="store_true", help="전체 종목 업데이트")
    group.add_argument("--symbols", nargs="+", help="특정 종목만 (예: AAPL MSFT)")

    # 필터링 옵션
    parser.add_argument(
        "--market",
        choices=["NYSE", "NASDAQ", "ETF", "ALL"],
        default="ALL",
        help="시장 필터 (기본값: ALL)"
    )
...
def main(argv=None) -> int:
    ...
    print(f"[us_weekly_update] skip_latest_week={skip_latest_week} (ET weekend -> False)")

    print(f"[us_weekly_update] 기준일: {end_date}")

    # Load symbols
    if args.all:
        symbols = load_us_symbols_with_details(
            engine=engine,
            market=args.market,
            top=args.top
        )
        print(f"[us_weekly_update] 처리 대상: {len(symbols)}개 종목")
    else:
        # 특정 종목만 - us_symbol_master에서 market 정보 조회
        symbols = []
        skipped = []
        for sym in args.symbols:
            sql = "SELECT symbol, name, market FROM us_symbol_master WHERE symbol = :symbol AND status = 'ACTIVE'"
            with engine.connect() as conn:
                ...
                if row:
                    ...
                    print(f"Warning: {sym} not found in us_symbol_master (or not ACTIVE), skipping", file=sys.stderr)
                    skipped.append(sym)

        if skipped:
            print(f"[us_weekly_update] Skipped {len(skipped)} symbols: {', '.join(skipped)}")
        print(f"[us_weekly_update] 처리 대상: {len(symbols)}개 종목")

    if not symbols:
        print("[us_weekly_update] 처리할 종목이 없습니다")
        return 0

    # Build US weekly pipeline
    pipeline = build_us_weekly_pipeline(cfg)

    # Initialize saver (US weekly table)
    weekly_cfg = cfg.get('indicators_us_weekly', cfg.get('indicators_weekly', cfg.get('indicators'))) or {}
